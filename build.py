...
if (subprocess.run(["javac","-encoding","utf8","-cp",(";" if os.name=="nt" else ":").join(ml),"-d","build"]+jfl).returncode!=0):
	sys.exit(1)
with zipfile.ZipFile("build/fischertechnic_instruction_builder.jar","w") as zf:
	...
	for r,_,fl in os.walk("rsrc"):
		...
		for f in fl:
			if (f[-4:]==".mdl"):
				...
				zf.write(r+f)
if ("--run" in sys.argv):
	subprocess.run(["java","-Dfile.encoding=UTF8","-jar","build/fischertechnic_instruction_builder.jar"])
